...
(function () {
    ...
    function parseStart(val){ return val ? new Date(val) : null; }
    function buildEndDate(endDateVal, endTimeVal){
        ...
        var hh = 23, mm = 59; // default if time not set
        if (endTimeVal) {
            var t = endTimeVal.split(':');
            hh = parseInt(t[0],10); mm = parseInt(t[1],10);
        }
    ...
    function init() {
        ...
        form.addEventListener('submit', function(e){
            ...
            if (endT.value && !endD.value) {
                e.preventDefault();
                endD.setCustomValidity('Пожалуйста, укажите дату окончания (задано время).');
                if (typeof form.reportValidity === 'function') form.reportValidity();
                endD.focus();
                return;
            }
            ...
            if (startDt && endDt && endDt < startDt) {
                e.preventDefault();
                endD.setCustomValidity('Дата/время окончания не может быть раньше начала.');
                if (typeof form.reportValidity === 'function') form.reportValidity();
                endD.focus();
                return;
            }
