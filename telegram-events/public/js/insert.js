document.addEventListener("DOMContentLoaded", () => {
  ...
  if (typeof window.DateTimeLogic === "undefined") {
    console.error(
      "⚠️ DateTimeLogic not found. Load /js/datetime-logic.js before app.js",
    );
  ...
  addForm.addEventListener("submit", async function (e) {
    ...
    try {
      ...
      var endDateV =
        endDateInput && endDateInput.value ? endDateInput.value : "";
      var endTimeV =
        endTimeInput && endTimeInput.value ? endTimeInput.value : "";
      ...
      if (!name || !category || !startStr || !place) {
        setSubmitting(false);
        showErrorBanner(
          "Пожалуйста, заполните обязательные поля: Название, Категория, Время начала, Место.",
        );
        return;
      }
      ...
      if (
        ...
        typeof DateTimeLogic.parseStart === "function"
      ) {
      ...
      if (
        window.DateTimeLogic &&
        typeof DateTimeLogic.buildEndDate === "function"
      ) {
      ...
      if (endDateTime) {
        newEventData.endDateTime =
          firebase.firestore.Timestamp.fromDate(endDateTime);
      }
      ...
      if (imgInput && imgInput.files && imgInput.files.length > 0) {
        ...
        if (!isImage || !isSmall) {
          setSubmitting(false);
          showErrorBanner(
            "Картинка должна быть формата image/* и меньше 5 МБ.",
          );
          return;
        }

        var storageRef = storage.ref(
          "eventImages/" + eventRef.id + "/" + file.name,
        );
        try {
          ...
          setSubmitting(false);
          showErrorBanner(
            "Не удалось загрузить изображение. Проверьте размер/тип файла и попробуйте снова.",
          );
          return;
        }
      }
      ...
      try {
        if (window.Telegram && Telegram.WebApp)
          Telegram.WebApp.showAlert("Событие добавлено!");
      } catch (_e) {}
      addForm.reset();
