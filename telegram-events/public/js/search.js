...
(function () {
    ...
    function showError(msg) {
        ...
        try { Telegram.WebApp.showAlert(msg); } catch (_) {}
    }
    function hideError() {
        const b = document.getElementById('formError');
        if (!b) return;
    ...
    async function runSearch(e) {
        ...
        const placeQ   = (document.getElementById('searchPlace').value || '').trim().toLowerCase();
        ...
        try {
            ...
            let q = db.collection('events').orderBy('startDateTime', 'desc').limit(200);
            if (cat) {
                q = db.collection('events').where('category', '==', cat).orderBy('startDateTime', 'desc').limit(200);
            }

            const snap = await q.get(q);

            const tagsArr = tagsIn
                ? tagsIn.split(',').map(t => t.trim().toLowerCase()).filter(Boolean)
                : [];
            ...
            snap.forEach(doc => {
                ...
                if (tagsArr.length) {
                    const docTags = Array.isArray(d.tags) ? d.tags.map(x => (x || '').toLowerCase()) : [];
