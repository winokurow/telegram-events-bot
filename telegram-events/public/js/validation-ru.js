...
(function () {
    ...
    function ruMessage(el) {
        ...
        if (v.typeMismatch && el.type === 'email')  return 'Введите корректный e-mail в поле «' + label + '».';
        if (v.typeMismatch && el.type === 'url')    return 'Введите корректный URL в поле «' + label + '».';
        if (v.patternMismatch)                      return 'Неверный формат в поле «' + label + '».';
        if (v.tooShort)                             return 'Слишком короткое значение в поле «' + label + '». Минимум: ' + el.minLength + '.';
        if (v.tooLong)                              return 'Слишком длинное значение в поле «' + label + '». Максимум: ' + el.maxLength + '.';
        if (v.rangeUnderflow)                       return 'Значение в поле «' + label + '» должно быть ≥ ' + el.min + '.';
        if (v.rangeOverflow)                        return 'Значение в поле «' + label + '» должно быть ≤ ' + el.max + '.';
        if (v.stepMismatch)                         return 'Недопустимый шаг значения в поле «' + label + '».';
        return '';
    }
    ...
    function init(formId) {
        ...
        if (!form) {
            console.warn('[validation-ru] Форма не найдена: id=' + (formId || 'addEventForm'));
            return;
        }
        ...
        form.addEventListener('input', function (e) {
            var el = e.target;
            if (el instanceof HTMLInputElement || el instanceof HTMLTextAreaElement) setFieldValidity(el);
        });
    ...
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', function(){ init('addEventForm'); });
    } else {
        init('addEventForm');
    }
})();
